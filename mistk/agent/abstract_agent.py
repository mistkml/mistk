...
class AbstractAgent (metaclass=ABCMeta):
    ...
    def initialize(self, objectives, props, hparams):
        ...
        :param objectives: The objectives of this agent.
            Possible values include 'learning'.
    ...
    def _do_initialize(self, objectives: list, props: dict, hparams: dict):
        ...
        :param objectives: The objectives for the agent.
            Possible values include 'learning'
        :param props: A dictionary that is parsed from a JSON string.
            These are settings that are passed from the ecosystem,
            but are not really considered hyperparameters. They are not used
    ...
    def do_initialize(self, objectives: list, props: dict, hparams: dict):
        ...
        :param props: A dictionary that is parsed from a JSON string.
            ...
            (e.g., where should heartbeats be sent and how often, etc).
        :param hparams: A dictionary that is parsed from a JSON string.
            These are hyperparameters that are used by the agent's learner.
        """
        pass
    ...
    def _do_build_model(self, path: str = ''):
        ...
        """
        if path:
            logger.info(f"Will try to load/build models from {path}")
            self.do_build_model(path)
        else:
            logger.info(f"Will create a brand new model")
    ...
    def _do_agent_registered(self, agent_cfg: dict):
        ...
        try:
            ...
            self.ready()
            logger.info('This agent is registered')
        except Exception as ex: #pylint: disable=broad-except
    ...
    def _do_episode_started(self, episode_cfg: dict):
        ...
        try:
            logger.info("Calling do_episode_started method.")
    ...
    def _do_get_action(self, obs: Dict[str, List[Union[int, float]]]) -> Dict[str, List[Union[int, float]]]:
        ...
        :return: A dict of lists. Each list contains the int/float actions an
                 agent should take.
        """
        logger.debug("_do_get_action started")
        try:  # Calls the child's agent so it can get actions from the learner
            logger.info(f"Calling do_get_action method using {obs}")
    ...
    def _do_replay_action(self, obs: Dict[str, Union[List[Union[int, float]], bool]]):
        ...
        """
        logger.debug("_do_replay_action started")
        try:
    ...
    def _do_episode_stopped(self):
        ...
        try:
            logger.info("Calling do_episode_stopped method.")
